...
def get_time_series_membrane(dt, T, fname, x, y, z):
    ...
    x_min = x - 0.5e-5; x_max = x + 0.1e-5
    y_min = y - 0.5e-4; y_max = y + 0.1e-4
    z_min = z; z_max = z + 0.04e-5
...
def plot_3D_concentration_neuron(res, T, dt, fname):
    ...
    # new mesh
    x_M = 4.674154869123957e-4
    y_M = 2.208589275930137e-4
    z_M = 1.5693481199926018e-4

    # 0.05 um above axon A (ECS)
    x_e = x_M; y_e = y_M + 0.1e-4; z_e = z_M
    # mid point inside axon A (ICS)
    x_i = x_M; y_i = y_M - 0.1e-4; z_i = z_M
    ...
    print("Na_n", Na_i[0], Na_i[-1])

    print("K_e", K_e[0], K_e[-1])
    print("K_n", K_i[0], K_i[-1])

    print("Cl_e", Cl_e[0], Cl_e[-1])
    print("Cl_n", Cl_i[0], Cl_i[-1])
...
def plot_3D_concentration_glial(res, T, dt, fname):
    ...
    x_M = 4.813723702751168e-4
    y_M = 0.4791020242777646e-4
    z_M = 0.6595906191667912e-4

    # 0.05 um above axon A (ECS)
    x_e = x_M; y_e = y_M; z_e = z_M + 0.1e-4
    # mid point inside axon A (ICS)
    x_i = x_M; y_i = y_M; z_i = z_M - 0.1e-4
